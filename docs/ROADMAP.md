...
| **v0.15.5** | ✓ Quality-of-life upgrades | Every redirect to `/login` carries its origin as a validated `?next=`, so signing in returns to the page — including for a shared link, whose signed-out recipient routes through the login first. A per-tab stash bound to the user, with a 3h TTL, brings the `/ratings` and `/watchlist` filters back with it. The share sheet now offers the title's deep link beside the PNG. Air dates become `Date` columns, and the anime and media heroes name the end date of anything airing now. |
