...
| **v0.15.6** | Test & CI setup | Branch protection repointed at the checks that actually run, so the suite gates a merge rather than a pair of context names nothing produces. Workflows scope a push to the component it touched while keeping pull requests whole, cancel superseded runs, and gain the two gates they lack: a production frontend build and the commit-gate regression suite. Dependencies split into runtime, dev tooling and a shared CPU torch pin. |
