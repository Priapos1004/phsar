...
| **v0.15.5** | ✓ Quality-of-life upgrades | Involuntary exits to `/login` carry a validated `?next=` and a per-tab filter stash, so signing in lands where you were. Returning from a detail page centres the card you clicked, expansion and page offset included. Air dates become `Date` columns and the heroes name when an airing title ends. The share sheet offers a deep link beside the PNG. Admin: job-detail changes rank by what drifted, and each job kind's health window matches its cadence. |
