...
| **v0.15.5** | ✓ Quality-of-life upgrades | Signing in after an involuntary exit lands where you were (`?next=` plus a per-tab filter stash), and returning from a detail page centres the card you clicked. The watchlist gains a per-anime **readiness** verdict — start-tonight versus airing or still waiting — as a Status filter and a card badge. Attribute pills aggregate per attribute; air dates become `Date` columns; the share sheet offers a deep link. Admin: job-detail changes rank by what drifted, health windows match each kind's cadence. |
