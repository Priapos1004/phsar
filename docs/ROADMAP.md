...
| **v0.15.5** | ✓ Quality-of-life upgrades | Signing in after an involuntary exit lands where you were, and returning from a detail page centres the card you clicked. The watchlist gains a per-anime **readiness** verdict (start-tonight versus airing or waiting) and a **watchtime** filter, with the runtime on every card. Attribute pills aggregate per attribute; air dates become `Date` columns; the share sheet offers a deep link. Admin: job-detail changes rank by what drifted, health windows match each kind's cadence. |
