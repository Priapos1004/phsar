...
| **v0.15.4** | Efficiency improvements + Claude Code setup *(in progress)* | Every change measured against a dev DB restored from a prod dump. Query and payload work across search, admin stats and the wide list endpoints: dead and PK-duplicating indexes dropped, `alembic check` in CI, gzip, flat projections. Frontend fetches gained caching and visibility gating, and the Jobs Log list stopped shipping detail-only sweep diffs. Separately, the pre-commit review pipeline became a `PreToolUse` hook with its own tests, and the docs were re-homed so each fact has one owner. |
