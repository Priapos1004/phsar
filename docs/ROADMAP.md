...
| **v0.15.5** | ✓ Quality-of-life upgrades | Involuntary exits to `/login` carry a validated `?next=` plus a per-tab filter stash, so signing in lands back where you were. Returning from a detail page centres the card you clicked, list expansion and page offset included. Air dates become `Date` columns and the heroes name when an airing title ends. The share sheet offers a deep link beside the PNG. Job-detail changes rank by what actually drifted. |
