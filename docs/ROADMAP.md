...
| **v0.15.6** | Test & CI setup | Branch protection repointed at the checks that actually run, so the suite gates a merge rather than context names nothing produces. Workflows scope a push to the component it touched, keep pull requests whole, cancel superseded runs, and gain a frontend production build plus the commit-gate suite. Dependencies split by who installs them. The split-curation queue, its backfiller and the untested endpoints gain tests. Models move to SQLAlchemy 2.0 `Mapped[]`, and mypy checks `app/` in CI. |
