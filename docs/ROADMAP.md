...
| `animation_quality` | Enum | no | low / normal / good / very_good |
...
## Feature 12: Sharing ✓ shipped (v0.15.2)

### Concept
Let a user show a friend an anime — how they rated it, or just what it is — over whatever messenger they already use.

### Key Decisions
- **An image, not a share link.** A hosted share page is the obvious design and was rejected: a temporary link expires (so the thing you sent your friend rots), and a permanent one is a public endpoint that can be enumerated or scraped to harvest users' rating data. A PNG the user saves is permanent, works in every messenger, and needs no public surface, no stored share state, and no expiry policy.
- **A purpose-built card, not a screenshot of the page.** The export is a fixed 1080×1350 portrait composition — the aspect messengers show uncropped. Screenshotting the page would inherit its responsive layout, so the same rating would export differently from a phone than from a desktop.
- **Themed, and built from the app's own components.** The card is the app's visual sandwich (themed page gradient behind a white elevated card), which is *why* the on-page quality radar and descriptive pills can be reused verbatim — their colours are already tuned for that white surface. One implementation, so the card and the page can't drift.
...
- **A second "info" card for anything unrated**, showing catalog facts and the synopsis where the rating card puts the radar. Recommending a show you *haven't* watched is a large part of what sharing is for, and it's the only card a guest can ever produce — so the variant is chosen by whether a rating exists, never by account type, and the button is never disabled for lack of one. Rated users get a toggle. Same shell for both, so two exports of the same app can't read as two products. The `--`-for-missing rule follows the attribute decision above for the same reason.
- **Spoiler protection does not gate sharing.** Asking to share is asking to see — the card can't spoil a show the user just requested a description of.
...
One row per release, saying what shipped — not why, and never with a measurement
in it. The reasoning and the numbers live in that release's compound-doc, indexed
in [compound-docs/INDEX.md](../compound-docs/INDEX.md). `/update-docs` owns the
length target.
...
| **v0.14.9** | ✓ Cleanup sweep | Themed Tooltip component. Admin "Dismissed decisions" history with delete-to-resurface. Per-check stabilizing breakdown, threshold lowered to 3. Probe-attached media persisted (`result_summary` v6) + job-detail audit polish. "Back to job" nav origin. |
...
| **v0.14.12** | ✓ Ratings page (anime-level list + statistics) | New `/ratings` page replacing the `/statistics` placeholder: anime-level list as a score-band cover grid or sortable table, with genre/season/age filters, plus ECharts statistics covering score distribution, You-vs-MAL alignment, genre/studio breakdowns and watch-time trends. All client-side off one wide `/ratings/scores` projection — no per-user stats endpoint and no scipy. `SegmentedControl` extracted. |
| **v0.14.13** | ✓ Ratings & reliability bug-fixes | Rating UX: `AttributeSelect`/`RatingNeighbors` shared across single and bulk, actual-watch-time stats for every watch status, guards for not-yet-aired media. Search: case-folded embeddings make it case-insensitive; scraper waivers for popular short-form shows. Reliability: `update_sweep` gains a circuit breaker so a total MAL outage aborts the run instead of holding maintenance for hours. |
| **v0.14.14** | ✓ Scrape API migration: Jikan → official MyAnimeList API v2 | `jikan_scraper`→`mal_scraper` on `api.myanimelist.net/v2`: header auth, one call per node, and a value-translation layer that keeps the DB enums, indexes and filters stable. Search roots are re-fetched via detail because MAL's `q=` omits `related_anime` and fragments franchises. Anime MAL score now averages over the main story only, not every media. |
| **v0.15.0** | ✓ Watchlist backend + UI | Per-user watchlist reworked to one list (tag) per entry — the `WatchlistTag` junction dropped — plus an immutable default list and a 1–3 priority. New `/watchlist` page: entries view with grain toggle, priority-band grid and sortable table, and list management. Colored bookmarks wherever a media or anime shows. Deliberately does not touch the spoiler frontier: want-to-watch is not watched. |
| **v0.15.1** | ✓ Watchlist analytics + ratings ↔ watchlist coupling + upcoming sweep | Watchlist **Statistics** subtab and an admin Overview card. Rating or rewatching a listed media offers an inline remove-from-watchlist toggle; `/ratings` gains the anime/media grain toggle. New `upcoming_sweep` JobKind reuses the seasonal dispatcher to pre-scrape the next season. |
| **v0.15.2** | ✓ Shareable cards | A share icon on the anime and media heroes opens a dialog holding a generated 1080×1350 PNG on the active theme's gradient. Two variants off one shell, picked by whether the title is rated: the rating card, and an info card for anything unrated — which is also every guest. An image rather than a hosted link, so nothing expires and no public endpoint exposes users' ratings. Frontend-only. |
| **v0.15.3** | ✓ Quality-of-life upgrades + little fixes | Jobs Log detail cells for the sweep and restore kinds, with `payloadSummary` extracted and tested. Ratings stats gain a **Categories** subtab (season and age-rating breakdowns) and steadier chart behaviour. Sweep gains a 180-day archival tier splitting the long tail. Backups record the Alembic revision they carry, so a schema-stale dump stops reading as restorable. |
| **v0.15.4** | ✓ Efficiency improvements + Claude Code setup | Every change measured against a dev DB restored from a prod dump. Query and payload work across search, admin stats and the wide list endpoints: dead and PK-duplicating indexes dropped, `alembic check` in CI, gzip, flat projections. Frontend fetches gained caching and visibility gating. Separately, the pre-commit review pipeline became a `PreToolUse` hook with its own tests, and every documented fact was given a single owner. |
...
| **v0.15.6** | Test & CI setup + rated coverage | Branch protection repointed at the checks that run; workflows scope a push to what it touched, keep PRs whole, and gain a frontend build plus the commit-gate suite. Dependencies split by installer. The split queue, its backfiller and untested endpoints gain tests; models move to SQLAlchemy 2.0 `Mapped[]` with mypy in CI. New **rated coverage** marks how much of an anime you have rated on search and `/ratings` cards — gold for the main story, obsidian for all, over aired entries only. |
