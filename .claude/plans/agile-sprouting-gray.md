...
| **v0.14.13** | Ratings bug-fixes & TODO cleanup | **Ratings:** shared `AttributeSelect` + `RatingNeighbors` across single + bulk (✕-to-clear, `not_applicable` made auto-set-only, live higher/lower neighbor color-coding; 3D-animation/ending-quality enums retuned); watch-time stats credit *actual* time watched every status (`RatingScoreItem` gains `episodes_watched` + `duration_seconds`, drops full-runtime `total_watch_time`); guards block rating not-yet-aired media, disable "Completed" with no episode total, and clamp `episodes_watched` client + server; bulk note pinned to the chronologically-last main (order-invariant) and not-yet-aired excluded with a warning. **Search/scraper:** mobile-tappable search-bar buttons + anime-view partial filter-clear; case-insensitive search (case-folded embeddings, one-time `reembed_all_embeddings`); scraper waivers admit popular short-form shows + feature-length ONAs; recaps (`full_story`) demoted to summary; admins exempt from the daily scrape cap. **Polish:** job bell — clickable finished rows, dismissible badge, completion toasts (new global `ToastHost`); related-media carousel centers on the current entry; rating score circle fits "10.00"; embedding tqdm log spam silenced. |
