...
| **v0.15.1** | ✓ Watchlist analytics + ratings ↔ watchlist coupling + upcoming sweep | Watchlist **Statistics** subtab + admin Overview watchlist card & 7d activity counter. Ratings↔watchlist coupling: a first rating or rewatch of a listed media offers an inline auto-checked remove-from-watchlist toggle; anime/media grain toggle on `/ratings`. New `upcoming_sweep` JobKind reusing the seasonal dispatcher to pre-scrape the **next** season (Wednesdays in the last month of the quarter), plus graceful direct-MAL-id 404s. Cosmetic/QoL polish across search + watchlist, and adopted ruff 0.16's curated ruleset. |
| **v0.15.2** | ✓ Shareable cards | Share icon beside the watchlist bookmark on the anime + media heroes opens a dialog holding a generated **1080×1350 PNG** on your active theme's gradient. **Two variants off one shell**, picked by whether you've rated the title: the rating card (score, 5-axis radar, 6 pills) and an **info** card (airing badges, catalog facts, genre/studio chips, synopsis) for anything unrated — which is also every guest. Rated viewers get a toggle; each variant builds once per dialog. An image rather than a hosted link so nothing expires and no public endpoint exposes users' ratings (Feature 12). **Frontend-only** — MAL's CDN allows cross-origin reads, so no image proxy; rasterized with `modern-screenshot`. `EChart` gained an `onReady` paint signal; `hasAnyAttribute` / `meanScore` / `triggerBlobDownload` extracted so the card can't drift from the page. iOS saves via the share sheet, its only path to Photos. |
| **v0.15.3** | ✓ Quality-of-life upgrades + little fixes | Jobs Log: `upcoming_sweep` and `restore` rows rendered a blank Detail cell; both season sweeps record the season swept; backup rows show the dump size. `payloadSummary` + row-tint helpers extracted to `jobSummary.ts` and finally tested. Ratings stats **Categories** subtab: season + age-rating breakdowns beside genre/studio, season/age labels clicking through to the filtered grid. Cumulative watch time scales to its window and carries the running total in from the left edge (a month with no ratings drew nothing). `EChart` gained a resize guard + `resetKey` so animations stop being cut short. Sweep: **180-day archival tier** splitting the 90-day long tail for media over a decade old. Backups record the **Alembic revision they carry** (free, off the existing hash stream), so a schema-stale dump stops reading as restorable — and a restart with none on the live schema enqueues one. Anime search: a filter no longer rescopes the score it ranks by — a franchise credited to a studio only through a side story sorted last behind lower-scored shows. |
| **v0.15.4** | Efficiency improvements *(in progress)* | Dev DB rebuilt on `pgvector/pgvector:pg17` from a prod dump, so every change below is gated on a before/after at real shape plus a proof of identical results. **Section filters survive the detail round-trip** — `/anime` and `/media` are siblings of the list sections, so a card click unmounted the section layout and its `onDestroy` wiped the filter you were returning to; replaced by one root-layout rule (`filterLifecycle`) plus per-tab sessionStorage (`persistedFilter`). **Migration hygiene:** `alembic check` was failing and would have dropped the sweep's own indexes — now declared in metadata, with `alembic check` in CI; 28 dead or PK-duplicating indexes dropped. **Search:** the anime genre filter stops costing more per genre chip; the 384-float embedding left the GROUP BY; the query embedding is memoized. **Admin:** the sweep-tier card's 18 SubPlans became one pass; `/filters/options` stopped discarding four medians. **Payload:** gzip (7× on list endpoints, backup download opted out) and flat projections for `/ratings/scores` + `/watchlist/items` (7 round trips → 1), which surfaced tied rows reshuffling between loads. Frontend fetch behaviour still to come. |
