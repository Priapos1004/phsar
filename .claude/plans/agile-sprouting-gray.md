...
| **v0.14.2** | ✓ Split candidates + sweep + scrape stability | Third detection pass: admin-reviewed split candidates for disjoint franchise chains under one anime row (Toaru Index↔Railgun, BNHA↔Vigilante, Overlord+Eminence). Media UUIDs preserved across split — user ratings stay attached, only the anime aggregation shifts. Strict WALK/TERMINAL BFS state machine prevents cross-franchise contamination; TERMINAL nodes fetch `/relations` so split detection sees full sequel chains. Nightly `update_sweep` step 1 also refreshes non-volatile metadata + reclassifies the umbrella when title-affecting drift is detected. Sweep detects genre/studio drift (safe adds applied, rest logged for admin review). MAL synopses strip `[Written by ...]` / `[Source: ...]` credits. Restore records itself as a Job row. Dev DB helper scripts under `phsar/scripts/` (read-only by default, `--apply` for mutations). TV substance gate lowered to 10 min. `CLAUDE.md` split into per-tree files. |
| **v0.14.3** | ✓ Admin page rework + detail-page UX | `/admin` restructured around `?tab=` switcher: Overview / Jobs Log / Curation. Overview surfaces catalog totals, 7d job health by kind, 7d activity counters. Jobs Log: paginated all-jobs listing with status/kind/user/date filters and seasonal_sweep child clustering under their parent. Pending merge + split candidates surface in the admin bell as a pinned reminder. Related-media carousel sorted chronologically + "you are here" marker on detail pages. "Back to library" button with origin propagation across navigation. Distinct browser tab title per route. Daily cap of 50 `user_scrape` submissions per user (rolling 24h) on top of the existing concurrent cap. Dismissed split candidates no longer re-flag on next detection. |
| **v0.14.4** | ✓ Merge-candidate detector fix (sidecar-as-truth) | `media_relation_edges` becomes the single source of truth for the `relation_link` signal. All three trigger sites (save, `update_sweep`, backfill) converge through `MergeCandidateDAO.get_cross_anime_relation_pairs` + `find_cross_anime_relation_pairs` service wrapper. Allowlist tightened to strong-link relations (`sequel`, `prequel`, `alternative_version` via shared `ALT_CHAIN_EDGES` with the classifier); weak relations (`spin-off`, `side_story`, `parent_story`, `summary`, `full_story`, `other`) no longer fire as merge signals. `backfill_merge_candidates` now runs the relation_link sidecar sweep too, so admin can surface Evangelion-shape pairs (NGE TV ↔ Rebuild Movies scraped in separate jobs) via the existing manual backfill endpoint. Invariant tests pin the allowlist contract. |
| **v0.14.5** | ✓ Sweep observability (per-job detail page, relaxed drift apply) | `update_sweep` writes a v3 `result_summary` (per-media field diffs + per-anime umbrella diffs + aggregated `unknown_genre_tags`) so admins can audit every nightly mutation via a new `/admin/jobs/[uuid]` detail page (header, counters, filterable Field/Was/Now diffs by tone, umbrella reclassifications). Genre + studio drift now auto-applies additions AND removals — the audit log is the rollback path. Unknown genre tags skip the apply and surface as an amber row tint + inline subline on the Jobs Log so admin can update the seeder. Jobs Log gains a live-ticking Duration column + click-through to the detail page (gated to update_sweep v2+). Admin Overview gains a SweepTiersCard showing where every anime sits in the refresh cycle. Per-kind `jobs.version` column + `JOB_KIND_VERSIONS` registry + `make_job()` helper so future result_summary shape changes don't break historical rows. JobWorker hardened so a downstream crash (e.g. datetime-in-JSONB) can never strand a job in `running` — pre-captured job_uuid + wrapped rollback + outer catch-all route every failure through `mark_failed`. Job health card stops counting system retryables since the bell's retry button only fires on user-owned rows. |
| **v0.14.6** | ✓ Backup retention, naming & restore-link | `apply_retention` reworked into three pools (archival manual+cron, relevance-based pre-restore, capped uploads); `_latest_per_sunday` fixes the old `[:8]` slice so each distinct Sunday keeps only its latest dump. Admin can rename a backup (`PATCH /admin/backups/{filename}`) — a non-empty name pins it against auto-deletion (kept on top of the rolling window, doesn't consume slots), blank/Remove-name unpins. Restore now links the pre-restore snapshot to the restored dump via sidecar `restored_to`; the current row shows "Previous state saved as …" and the pre-restore tied to the current state is retention-pinned. Nightly backup job re-verifies every dump's integrity (`pg_restore --list`) before retention so on-disk corruption stops listing as `ok` (closes the v0.13.0-flagged stale-integrity gap). All metadata in `.meta.json` sidecars — no DB migration. |
