...
| **v0.14.14** | ✓ Scrape API migration: Jikan → official MyAnimeList API v2 | `jikan_scraper`→`mal_scraper` on `api.myanimelist.net/v2` (header auth, one call per node, value-translation layer keeps DB enums/indexes/filters stable, `Crossover` dropped, self-healing sweep). Post-migration fixes: search roots re-fetched via detail (MAL's `q=` omits `related_anime` — had fragmented franchises into single-media rows); sweep deletes + blacklists Hentai flips (v7); enum-coercion self-heal crash fixed; substance-gate relaxation scoped to the main chain (short-form franchises no longer wrongly demoted by off-chain full-length specials); high-episode short-form total-runtime waiver (Saiki K. S1 and other flagship short-form seasons now anchor + classify as Main, not Side Story). |
