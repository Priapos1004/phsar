...
| **v0.14.13** | Ratings bug-fixes & TODO cleanup | Search bar: filter icon left + search-trigger button right (mobile-tappable), Relation Type filter hidden in anime view, view toggle does a partial clear (carries query + categorical filters + 0–10 score, drops view-relative ranges). Rating attributes: `✕`-clear to deselect + shared `AttributeSelect` so bulk/media can't drift, `not_applicable` made an auto-set-only sentinel (now covers ending type too), 3D animation rebucketed to none/rare/medium/heavy + `ending_quality` gains neutral, shared `RatingNeighbors` brings the "how you rated similar titles" helper to bulk with live higher/lower/differs color-coding. Admins exempted from the daily scrape cap. Scraper popularity waiver: widely-scored short-form shows (≥10k MAL scorers, e.g. *Love is Like a Cocktail* at 3 min/ep) that fail the 10-min duration floor now save as standalone anime — scoped to the weak-anchor save decision, not the relation-ranking substance gate. Scraper feature-length-ONA waiver: 1-episode ONAs with a ≥60-min runtime (theatrical films MAL labels ONA, e.g. *Bubble*) now save instead of being dropped at the episode floor — same weak-anchor-only scope. Search made case-insensitive: `generate_embedding` case-folds before encoding (the cased model gave `Kurokos` vs `kurokos` different vectors, reordering title results); one-time catalog re-normalization via `reembed_all_embeddings` behind `EMBEDDING_REEMBED_ON_STARTUP`. Classifier: recaps (MAL `full_story` edge, e.g. Kuroko Winter Cup movies) demoted to `summary` instead of riding a sequel/prequel bridge into `main`. `generate_embedding` silences the per-call tqdm bar (Coolify log spam). Job bell QoL: finished add-anime/backup rows are clickable (→ `/library/add` / `/admin?tab=backups`), opening the bell dismisses the badge even while a job is still fetching, and a green/red completion toast (new global `ToastHost`) fires when an add-anime or backup job finishes. |
