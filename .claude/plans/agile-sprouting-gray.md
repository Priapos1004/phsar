...
| **v0.15.3** | Quality-of-life upgrades + little fixes | Jobs Log: `upcoming_sweep` rows rendered a blank Detail cell despite carrying an identical `result_summary` (the branch was gated on a `seasonal_sweep` literal, now on the shared `PARENTING_KINDS` set); both season sweeps now record the targeted season (additive keys, no version bump) and backup rows show the dump size that was already in the summary. `payloadSummary` + the row-tint helpers extracted to `jobSummary.ts` so 5 kinds × 3 schema eras are finally unit-tested. Ratings **Categories** subtab (was Genres & Studios): breakdowns by **season** and **age rating** alongside genre/studio off a `TAG_DIM_KEYS` table, with season/age labels clicking through to the filtered ratings grid. Cumulative watch time reads its own span instead of anchoring at 0, and a clipped window now carries the running total in from its left edge (a month with no ratings drew nothing at all before). `EChart` no longer lets a container resize snap an in-flight animation. |
