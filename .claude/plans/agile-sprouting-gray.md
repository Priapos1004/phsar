...
## Feature 12: Rating Sharing (v0.15.2)

### Concept
Let a user show a friend how they rated an anime, over whatever messenger they already use.
...
| **v0.15.2** | Shareable rating cards | Share icon beside the watchlist bookmark on the anime + media heroes opens a dialog holding a generated **1080×1350 PNG** of your rating — cover, title, score, the 5-axis quality radar and the 6 descriptive pills, on your active theme's gradient. Save it, or hand it to the phone's native share sheet. An image rather than a hosted link so nothing expires and no public endpoint exposes users' ratings (Feature 12). **Frontend-only** — MAL's CDN allows cross-origin reads, so no image proxy; `ShareCard` reuses the on-page attribute viz verbatim, rasterized with `modern-screenshot`. `EChart` gained an `onReady` paint signal; `hasAnyAttribute` / `meanScore` / `triggerBlobDownload` extracted so the card can't drift from the page. |
