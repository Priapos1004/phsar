...
| **v0.14.14** | ✓ Scrape API migration: Jikan → official MyAnimeList API v2 | `jikan_scraper`→`mal_scraper` against `api.myanimelist.net/v2` (header auth, no OAuth). One MAL call per node (`/anime/{id}` bundles `related_anime`). Value-translation layer keeps DB enums/indexes/filters untouched. Self-healing sweep (`duration_seconds`, `aired_from`, `media_type`, season, `mal_url`). `Crossover` relation dropped (MAL v2 doesn't emit it). |
