...
| **v0.15.4** | Efficiency improvements *(in progress)* | Dev DB rebuilt on `pgvector/pgvector:pg17` from a prod dump so plans and timings are measured at real shape. **Section filters survive the detail round-trip** — `/anime` and `/media` are siblings of the list sections, so a card click unmounted the section layout and its `onDestroy` wiped the filter you were returning to; replaced by one root-layout rule (`filterLifecycle`) plus per-tab sessionStorage (`persistedFilter`), so a refresh keeps them too. Remaining scope is measurement-gated off a new performance baseline doc. |
