...
| **v0.15.1** | Ratings ↔ watchlist coupling + ratings-overview grain toggle | Rating a watchlisted anime → prompt to remove it from the watchlist (the Feature 1 coupling deferred from v0.15.0). Anime/media grain toggle on the `/ratings` overview (mirrors the watchlist overview's toggle). |
