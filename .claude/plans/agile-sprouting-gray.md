...
- **Retention runs after every backup job, not cron-only.** Both manual and cron dispatcher paths end with `apply_retention()`. Without this, manual creates pile up indefinitely on installs where cron is disabled (empty `BACKUP_CRON_TOKEN`) or rarely fires — the cron path was historically the only one wired to retention.
