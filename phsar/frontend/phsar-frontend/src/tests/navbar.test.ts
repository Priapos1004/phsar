...
describe('NavBar', () => {
	it('renders logo and brand text', () => {
		render(NavBar, { props: { isAuthenticated: false, onLogout: vi.fn() } });
	...
	it('shows user button when authenticated', () => {
		render(NavBar, { props: { isAuthenticated: true, onLogout: vi.fn() } });
	...
	it('calls onLogout when logout is clicked', async () => {
		...
		render(NavBar, { props: { isAuthenticated: true, onLogout } });
