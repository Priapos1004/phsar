...
- **Admin tab navigation**: `/admin` uses a `?tab=` query param to switch between sections (`tokens`, `curation`, `backups`). The tab list, default, and validation live at the top of `routes/admin/+page.svelte`; the tab UI itself is `lib/components/admin/AdminTabNav.svelte`. Adding a tab requires updating three places: `AdminTabKey` in `lib/components/admin/types.ts`, the `TABS` array + content cascade in the page, and the rendering branch
