...
- **admin/SweepTiersCard** — admin Overview card showing where every anime sits in the update-sweep cycle. Backend exposes 4 mutually-exclusive **cycle-membership** priority-cascade buckets (airing_now / stabilizing / weekly_cycle / long_cycle); the card renders them 1:1 (no fold). Membership not due-ness — the buckets reflect cadence membership, so counts stay stable across sweeps instead of emptying when a sweep refreshes a tier. Per-row horizontal share bar, count, percentage, tooltip with the underlying predicate
- **admin/JobDetailHeader / JobDetailCounters / MediaChangeCard / AnimeUmbrellaCard** — sub-components of the `/admin/jobs/[uuid]` detail page
  ...
  - `JobDetailCounters`: takes `version`; the `step1_failed` cell renders only for v4+ (older rows show "—", not a misleading `0`) and tints amber when `> 0`
