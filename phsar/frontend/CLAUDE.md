...
- **BackupsCard** — admin-only dump list
  ...
  - **Rename / pin** (pencil → inline `Input` + Save/Cancel; `PATCH /admin/backups/{filename}`): a non-empty name pins the dump against auto-retention and renders a "Pinned" badge; an inline "Remove name" button (or saving blank) unpins. `saveName(filename, name)` is the shared call behind `handleRename` (Save) and `handleUnpin` (one-click remove)
