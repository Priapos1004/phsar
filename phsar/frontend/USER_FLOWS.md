...
- Four stat cards sourced from `GET /admin/stats/overview`:
  ...
  - **Sweep tiers**: where every anime currently sits in the update-sweep cycle. Four rows in priority cascade — Airing now (emerald) > Stabilizing (amber) > Weekly recent main (sky) > 180-day cycle (indigo). Each row has a horizontal share-of-total bar, count, percentage, and tooltip paraphrasing the predicate. The backend exposes 5 mutually-exclusive buckets (priority cascade in `AnimeDAO.count_by_sweep_tier_priority`); the UI folds `long_tail + not_currently_due` into the "180-day cycle" row because long_tail is a transient pre-sweep state — anime in it get refreshed on the very next sweep and move to not_currently_due. Sum equals catalog total
