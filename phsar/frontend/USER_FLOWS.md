...
- Four stat cards sourced from `GET /admin/stats/overview`:
  ...
  - **Sweep tiers**: where every anime sits in the update-sweep cycle. Four rows in priority cascade — Airing now (emerald) > Stabilizing (amber) > Weekly cycle (sky) > 180-day cycle (indigo). Each row has a horizontal share-of-total bar, count, percentage, and tooltip paraphrasing the predicate. The backend exposes these as 4 mutually-exclusive **cycle-membership** buckets (`AnimeDAO.count_by_sweep_tier_priority`); the card renders them 1:1. Membership, not due-ness — a tier counts where an anime *belongs* (e.g. "Weekly cycle" = has a recent main), so a row doesn't empty itself when a sweep refreshes its members. Sum equals catalog total
...
- Columns:
  ...
  - **Detail** — for failed rows, the `error_message` in destructive color; for succeeded rows the dispatcher's `result_summary` rendered per-kind: user_scrape → "+N anime · +M media", update_sweep v3 → "N touched · X anime w/ dynamic · Y media w/ static · Z umbrella · W new attached" (older v1 rows fall back to the legacy "refreshed N anime · M changed · …" copy), seasonal_sweep → "N season entries · M new scrapes enqueued · K already known", backup/restore → filename
...
- **Counters grid** (v2+ jobs only): 9 stats from `result_summary.counters` — anime refreshed, anime w/ dynamic changes, anime w/ static changes, media w/ dynamic changes, media w/ static changes, umbrella reclassed, probes succeeded, probes failed, anime w/ new attach. Plus inline warning lines if `merge_detect_failed` or `cache_recompute_failed` fired (the catalog work still committed; only the post-sweep merge-detection / spoiler-cache recompute failed)
...
- **Anime umbrella changes section** (v2+ only): for each anime whose 7-field umbrella drifted, an `AnimeUmbrellaCard` shows the anime title (link to `/anime?uuid=<uuid>`, new tab), "anchor moved" / "embedding regen" badges when applicable, a Field/Was/Now table for the changed umbrella fields, and a list of per-media relation reclassifications (`mal_id=NNNN: old_rt → new_rt`)
...
- **Rename / pin**: pencil icon opens an inline name field (Save / Cancel). A non-empty name pins the dump so auto-retention never deletes it (admin actively chose to keep it); the row then shows a "Pinned" badge. While editing a named dump, a "Remove name" (pin-off) button clears the name in one click; saving a blank name does the same. `PATCH /admin/backups/{filename}` with `{name}`. Pinned dumps are kept on top of the normal rolling window — they don't consume the 14-recent / Sunday / cap slots.
