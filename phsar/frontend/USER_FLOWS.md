...
- **Failed-refresh / Failed-probe cards**: when `step1_failures` (v4+) or `probe_failures` (v5+) is non-empty, a card lists each skipped anime — title (link to `/anime?uuid=<uuid>`), `error_category` chip, and the error message. Both kept their old `last_checked_at` / `AnimeFreshness`, so the next sweep retries them — except a 404 (v8+), which means MAL deleted the entry: that row stamps its clock instead, raises a delete candidate, and carries a violet "Gone from MAL — delete candidate raised" link to the Curation tab (12.7) rather than a card of its own. A progress-divergence notice fires when `items_done < items_total` — v5 progress is media-grained (`items_total` = due media, `items_done` = refreshed media), so the gap is the media skipped because their anime failed step-1 refresh (probe failures don't widen it — their media committed; they show in the Failed-probe card). v2–v4 rows keep the anime-grained wording
