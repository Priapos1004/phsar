...
| `seed_demo_sweep_job.py` | **mutates** | Insert a realistic demo `update_sweep` job at the current registry version, built from REAL catalog rows so every link resolves, for evaluating the admin job-detail page + Jobs Log without waiting for a nightly sweep. Dry-runs by default and flags the pool-sliced lists whose cards would otherwise render empty; `--apply` inserts, `--delete --apply` removes (idempotent via a payload marker). The flags select which row tint to see in isolation — `--help` lists them and the module docstring argues the precedence. |
