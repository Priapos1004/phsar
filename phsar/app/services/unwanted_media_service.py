...
async def create_unwanted_media(db: AsyncSession, unwanted_media: set[tuple[int, str]]) -> None:
    logger.debug(f"DB session: {id(db)}")
    inserted = 0
    for mal_id, reason in unwanted_media:
        existing = await media_unwanted_dao.get_by_mal_id(db, mal_id)
        if existing:
            logger.info(f"Unwanted media with mal_id={mal_id} already exists (reason={reason}). Skipping.")
            continue
