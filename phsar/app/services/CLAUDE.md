...
- **`watchlist_service.py`** — the watchlist entry layer
  ...
  - **Bulk note goes on the chronologically-FIRST main media** — the mirror of `RatingBulkCreate` (which places it on the *last* main): a watchlist note ("start here / heads up") belongs on the earliest season, a rating note ("my take") on the latest. The target index comes from the shared `filter_service.select_note_target_index(media_list, latest=False)` (bulk rating passes `latest=True`) so the two can't drift. Priority + tag still apply uniformly via `_apply_fields` (the single field-mapping site shared by the create + update + bulk paths); the bulk loop overrides `note` per-media after that so only the target keeps it, and **every other entry keeps the note it already had** — bulk is also how a whole anime changes list or priority, so clearing there would destroy notes the request never mentioned. An **omitted** `note` (distinguished from an explicit null via `model_fields_set`) leaves all of them alone, which is what lets a client move an anime without naming a note. `get_watchlist_for_anime` returns the target's media uuid alongside the entries for the same reason: a client that re-derived the rule would prefill an edit box from one entry and have the write land on another, replacing that one's note
