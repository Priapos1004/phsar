...
- **Versioning:** every Job-construction site goes through `make_job(kind, **kw)` (`app/core/job_versions.py`), which stamps `job.version` from the `JOB_KIND_VERSIONS` registry. Frontend reads `job.version` to pick a parser for `result_summary`. Bump the integer per kind when its shape changes — `update_sweep` is at v3 after v0.14.5
...
- **v4 result_summary (v3 in v0.14.5, v4 in v0.14.7):** captures every change as an auditable diff so admins can inspect the per-sweep delta on `/admin/jobs/[uuid]` and roll back if MAL ever propagates a data bug at scale. Shape: `{counters, media_changes[], anime_umbrella_changes[], unknown_genre_tags[], step1_failures[], merge_detect_failed, cache_recompute_failed}`. Per-media entries carry old → new for every dynamic + static field write (via the `diff_sink` parameter threaded into `_apply_media_diff` / `_apply_metadata_diff`) plus the genre/studio M2M drift; umbrella entries carry the 7-field ReclassifyDiff via `anime_relation_service.umbrella_diff_to_log_entry`. Datetime values (e.g. `aired_to`) coerce to `isoformat()` at the diff-sink site via `_jsonable()` — the JSONB column's `json.dumps` rejects native datetime
- **Step-1 failure tracking (v4, v0.14.7):** `_try_step1_refresh` returns a typed `Step1Failure` (not bare `None`) on a refresh exception; the dispatcher increments `counters.step1_failed` and appends `{anime_uuid, title, error_category, error_message}` to top-level `step1_failures[]`. `error_category` reuses `job_worker.classify_error` so an MAL outage during a sweep tags the same `upstream_outage` as a job-level failure. Without this a sweep that dropped a chunk of its selection (e.g. 64 of 113 anime committed during a transient MAL outage) still rendered a clean `succeeded`; the bump to v4 lets the frontend tell a v4 `step1_failed: 0` (genuinely zero) from a v3 row that never tracked it
...
- **`spoiler_service.py`** — frontier algorithm + precomputed visibility cache in `user_visible_media`
  ...
  - **Restricted users are excluded** from the scoped recompute, both registration recompute paths, and the `backfill_spoiler_visibility` query (pinned to `spoiler=off`; never read the cache — the hide-mode read in `search.py` short-circuits when `spoiler_level != hide`)
