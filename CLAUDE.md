...
- **`/auth`** — `POST /auth/login` + `POST /auth/register` issue a JWT (`sub`, `role`, `exp`); `GET /auth/validate` checks one (kept for external/manual use — the frontend's navigation guard decodes `exp` locally instead; see Sliding session). `POST /auth/refresh` re-issues a fresh full-lifetime token to any **still-valid** token (an expired token 401s, same as `/validate`) — the sliding-session re-issue the frontend calls on activity (see Sliding session under Key Patterns). Bare `get_current_user` (not role-gated) so restricted/guest users keep their sessions alive too; role is re-read from the DB user
...
- `seasonal_sweep_dispatcher.py` — `seasonal_sweep` AND `upcoming_sweep` handler (separate; pure discovery pass). One function serves both kinds: it scrapes the current season for `seasonal_sweep` and the next season (`mal_scraper.next_season`) for `upcoming_sweep`, chosen by `job.kind`. Registered for both kinds in the lifespan. The `result_summary` records the targeted `season_year` + `season_name` alongside the counters — the counters alone can't say *which* season ran, which is the whole point of an upcoming-sweep row. The name goes through `mal_scraper.catalog_season_name`, the single owner of the MAL-lowercase → `SeasonType` vocabulary boundary (shared with `extract_information`'s season derivation), so a persisted JSONB summary can't drift from the enum
...
- **`AnimeDAO`**:
  ...
  - `select_due_media_for_sweep` — **media-level** four-tier sweep selection (v0.14.8): each atom is a direct predicate on the media row + its `MediaFreshness` sidecar (airing now / still stabilizing `stable_check_count < SWEEP_STABILIZE_THRESHOLD` / weekly recent main / long tail), so a still-airing umbrella's stable members are no longer dragged through a refresh every night. v0.15.3 gave the long tail a **per-row window** rather than a fifth tier: `due_long_tail` compares `last_checked` against `SWEEP_ARCHIVAL_DAYS` (180) for media that premiered over `SWEEP_ARCHIVAL_AGE_YEARS` ago and `SWEEP_LONG_TAIL_DAYS` (90) otherwise, via a `CASE` on the interval. One predicate, so the tiers stay independent — a separate fifth branch would need tier 4 narrowed by `not_(archival)` to avoid shadowing it, and that coupling is easy to forget. It also makes NULL correct for free: an undated media fails the `WHEN` and lands on the 90-day `ELSE`. Returns `Media` rows with the parent `Anime` + its full media set eager-loaded (the dispatcher groups by `anime.id`). `LIMIT` bounds MAL calls = media (the real 1 req/s cost unit). Replaced the anime-grained `select_due_for_sweep`. Backed by:
  ...
  - `count_by_sweep_tier_priority` / `count_media_by_sweep_tier_priority` — anime- and media-grained membership-bucket counts for the admin Overview SweepTiersCard toggle; both use the shared atoms (`_sweep_atoms` / `_media_sweep_atoms`) and the shared `_tier_bucket` CASE. `SWEEP_STABILIZE_THRESHOLD` (3), `SWEEP_LONG_TAIL_DAYS` (90), `SWEEP_ARCHIVAL_AGE_YEARS` (10) and `SWEEP_ARCHIVAL_DAYS` (180) are single constants shared by media + anime so the two grains can't drift. `archival_cycle` is an explicit CASE branch **before** the `else_` (which is `long_cycle`, so an appended branch would never fire) and sits after `recent_main`, so a franchise with a recent main season still reads as weekly even with decade-old members.
    ...
    - **The CTE is LEFT JOINed and its columns are NOT coalesced.** A media-less anime has no CTE row, so every atom reads NULL, every `WHEN` is not-true, and it falls to the `else_` (`long_cycle`) — correct. Coalescing `min_stable` to 0 looks like defensive tidying and would instead bucket it as `stabilizing_0`; `test_media_less_anime_falls_through_to_long_cycle` pins it. The `coalesce` *inside* the CTE is the unrelated case of a media with no freshness sidecar
    - `archival` is the **∀** quantifier — `MAX(aired_from) < cutoff`, "the newest thing this franchise aired is older than the cutoff" — where `airing_now`/`recent_main` are **∃** (`bool_or`). Same "inherits its most-urgent media's tier" rule, quantifier flipped because archival is the slow end of the cascade, so a `bool_or` here "for symmetry" would be wrong. `MAX` also drops the NULL hazard: `NULL < cutoff` isn't true, so an all-undated anime falls to `long_cycle`
    - The stabilizing total is further split into `stabilizing_by_check` (counts per `stable_check_count` 0…threshold-1) so the card can show the stabilization pipeline — media grain by the media's own count, anime grain by its least-settled member (`MIN` across its media); dynamic in the threshold
- **`media_projections.py`** — shared column blocks for the two wide list projections (`/ratings/scores`, `/watchlist/items`): the `media_*`/`anime_*` identity labels (`uuid`/`title`/`name_eng`/`name_jap`/`cover_image` exist on BOTH, so an unlabelled projection silently collapses each pair) plus the genre/studio name arrays. Those arrays are aggregated ONCE over the requesting user's media and LEFT JOINed — a correlated `array_agg` per output row instead makes the cost scale with the page, and grouping over the whole catalogue aggregates rows nobody asked for
- **`search_filters.py`** — shared filter/ordering helpers for media filters, anime selection, and anime post-aggregation (HAVING)
  - **Anime-view filters pick anime; they never rescope the aggregates.** `apply_anime_pre_filters` selects `Anime.id IN (SELECT media.anime_id WHERE <conditions>)` — one subquery, so all conditions hold for the same media row ("studio X + type TV" = one media is a TV by X) — rather than filtering the grouped rows. Those rows also feed `avg_score`/`avg_scored_by`/`total_episodes`/`media_count` and every HAVING filter, so narrowing them would make the shown score (and the no-query ordering derived from it) depend on which media the filter kept: an anime credited to a studio only through a side story (relation weight 0) would rank on an empty main set while its card shows the mean over all its main media
  - **Genre majority is a second, separate `Anime.id IN (…)` condition** in the same function (`_anime_genre_majority_condition`) — deliberately not folded into the subquery above, because it is not a same-media-row question but an aggregate over the anime's whole media set: every selected genre must be carried by a majority of its media (`count * 2 > total`, the threshold `filter_service._get_anime_majority_genres` uses to populate the dropdown). It must stay **one non-correlated pass** (per-(anime, genre) counts and per-anime totals computed once, joined, surviving pairs counted against the number of genres selected). The obvious alternative — a correlated majority-subquery per selected genre — is superlinear in genres, since each one both adds a SubPlan and widens the set every existing SubPlan is re-evaluated over, and this fires on ticking genre chips. The denominator is the anime's FULL media count: `test_genre_majority_denominator_survives_a_pre_filter` pins that a stacked studio filter can't shrink it, `test_multiple_genres_all_must_reach_majority` pins the AND-across-genres semantics
...
- **main.py** — app factory + lifespan
  - Startup sequence: seeders → embedding backfill → `JobDAO.reap_orphans` → `job_worker.register_dispatcher` (for each of `user_scrape`, `update_sweep`, `seasonal_sweep`, `upcoming_sweep` (same fn as seasonal), `backup`) → `job_worker.start()` → yield → post-yield backfills (**backup self-heal** → optional one-shot full re-embed when `EMBEDDING_REEMBED_ON_STARTUP` → relation-backfiller → merge-candidate backfill → split-candidate backfill). Post-yield backfills run in `asyncio.create_task` so /health responds during the ~14-min relation cold start; split detection runs LAST so it sees relation-backfiller's TERMINAL sidecars and merge-backfiller's collapsed pairs
    - `backup_service.ensure_up_to_date_backup` (v0.15.3) enqueues a `backup` job when no dump on disk has `status == ok` (the composite verdict = `integrity == ok` **and** `schema_current`). `docker/entrypoint.sh` runs `alembic upgrade head` before the app process, so after a migrating deploy every existing dump carries the previous revision and restoring one would roll the schema back — this closes the window where an install has no restorable backup until the next nightly cron. Post-yield because `list_backups()` may shell out `pg_restore --list` per sidecar-less dump; **first** among the post-yield passes because all of them are idempotent derived-data repairs that re-run every boot, so a pre-backfill dump loses nothing while a post-backfill one couldn't recover from a bad backfill. `schema_current is None` (legacy sidecar) reads as `status == unknown`, which does NOT satisfy the predicate; a `JobDAO.count_active_by_kind` guard stops rapid restarts stacking dumps; runs as a `cron`-source system job so it needs no new `BackupSource` member and stays out of every user's bell. `main.py` holds only the ordering — the policy lives in the service
...
- **Sharing** (v0.15.2): a share icon beside the watchlist bookmark on the anime + media heroes exports a **PNG** on the active theme's gradient. Two cards, chosen by whether a rating exists and never by account type: the **rating** card (cover, title, score, the 5-axis quality radar and the 6 descriptive pills) when the viewer has rated, and otherwise an **info** card (airing + story-complete badges, catalog facts, genre/age/studio chips, synopsis) — because recommending something you *haven't* watched is a large part of what sharing is for, and guests can never rate. A rated viewer can switch between the two in the dialog. Deliberately an image, not a hosted share link: a link would either expire or become a scrapeable endpoint exposing users' ratings, whereas a PNG the user owns is permanent and works in every messenger. **Entirely frontend** — no endpoint, no stored state, and no image proxy (MAL's CDN serves `Access-Control-Allow-Origin: *`, so a cover can be inlined without tainting the canvas). Everything the card shows is already in page state; the one network read is re-fetching the cover bytes to inline them. The button is **never disabled** — including for a media behind the viewer's spoiler frontier, since asking to share is asking to see. See the frontend CLAUDE.md for the capture pipeline
- **Section filter lifecycle** (v0.15.4): the `/ratings`, `/watchlist` and `/admin?tab=jobs` list controls live in module stores mirrored to per-tab **sessionStorage** (`lib/stores/persistedFilter.ts`), never the URL, and are wiped on logout and on an A→B user switch. One rule governs the reset, dispatched from a single `afterNavigate` in the root layout (`lib/utils/filterLifecycle.ts`): **a section's filter is cleared when navigating to a page that is neither inside that section nor a detail page** — `/anime` and `/media` are top-level *siblings* of the list sections, so a card click is a detour, not an exit. See the frontend CLAUDE.md for why this can't live in the section layouts (it used to) and for the version-envelope + sanitizer rules
...
- **Maintenance mode**: destructive ops flip `core/maintenance.py`'s `_active` flag
  ...
  - **Critical middleware ordering invariant**: `app.add_middleware()` *prepends* (`insert(0, ...)`), so LAST registered = OUTERMOST
    - Register `MaintenanceGateMiddleware` FIRST, then `GZipMiddleware`, then `CORSMiddleware` — CORS wraps both, so a 503 gets `Access-Control-Allow-Origin`
    - Wrong order → 503 has no CORS headers → browser blocks → `fetch()` throws TypeError → generic error instead of maintenance banner. Pinned by `test_maintenance_503_still_carries_cors_headers`
...
- **Response compression**: `GZipMiddleware(minimum_size=1000, compresslevel=6)`, registered between the maintenance gate and CORS (see the ordering invariant above). The JSON list endpoints are long runs of repeated field names and compress ~7×, which on a home connection to the VM is the dominant term in their latency; level 6 rather than starlette's default 9 because 9 spends real CPU on a 2-vCPU VM for a percent or two of size
  - **The backup download opts out** with `Content-Encoding: identity`. Starlette decides on size + content-type alone, so without it a multi-GB `pg_dump -Fc` archive — already zlib-compressed — gets streamed through gzip for ~0% gain, and loses its `Content-Length` (the browser's download progress goes indeterminate). Starlette skips compression whenever a content-encoding is already set, so that header IS the opt-out; RFC 9110 reserves `identity` for `Accept-Encoding`, making this the pragmatic idiom rather than a conforming header. `test_backup_download_opts_out_of_compression` fails if it goes missing
...
- Manual backups: admin panel → Backups card → "Create backup"
  ...
  - **Alembic revision stamping** (v0.15.3): each dump records the revision it *carries* in its sidecar (`alembic_revision`), parsed out of the `COPY public.alembic_version` block during the content-hash stream — zero extra subprocess, and reading it from the dump rather than the live DB is what makes it right for an **uploaded** dump. `list_backups_with_revision` reads the live revision once per call (`read_db_revision`, fails soft to None) and derives two non-persisted fields per row: `schema_current` (either side unknown → `None`, deliberately not `False` so a legacy dump is never accused) and **`status`**, the composite restorability verdict (`corrupt` > `outdated` > `unknown` > `ok`).
    - **`integrity` stays a pure file-corruption verdict** — `apply_retention` pins the most-recent-ok dump as its archival anchor, so folding the schema verdict in would leave retention anchorless right after every migration. The composite is a *separate* field rather than a UI concern: the Backups card, its sort, and the startup self-heal all decide on the same question, and while the frontend owned the composition the two disagreed — a revision-less dump rendered green `ok` while the self-heal had just judged nothing on disk restorable. `unknown` is therefore NOT `ok`: a dump nothing vouches for shouldn't be advertised as one you can rely on.
    - `GET /admin/backups` returns a `BackupListResponse` envelope (`db_revision` + `backups`) so the live revision comes from the server. Deriving it from "whichever dump is `schema_current`" made it unavailable exactly post-migration, when no dump matches — which is when the amber badge most needs to name what the dump is outdated *against*
...
- **Restart-triggered backups** (v0.15.3): a boot where no dump on disk is both intact and on the live schema enqueues one immediately (`backup_service.ensure_up_to_date_backup`, post-yield). This is why an unexplained `Scheduled`-source dump can appear minutes after a deploy — expected, and **one-time on the first v0.15.3 boot** since no existing dump records a revision yet. It's a system job, so it shows in the Jobs Log and in no user's bell. It deliberately does **nothing** when the live revision can't be read: `status` is a comparison against that value, so a dump taken then couldn't clear the condition either and every restart would re-dump
...
- **Backend Tests** (`backend-test.yml`): `pytest` against pgvector service container — every push/PR. Also runs **`alembic check`**, guarding two things at once: models and migrations agree (an index or column declared in only one is what makes the next `--autogenerate` propose a destructive diff), and the chain still replays from empty. It needs its own throwaway `migrationcheck` DB brought up by `alembic upgrade head` — run against the test DB it would compare `create_all`'s metadata to a schema built from that same metadata, and pass however far the migrations had drifted
