...
- **Vector search**: `paraphrase-multilingual-MiniLM-L12-v2` model, pgvector storage
  ...
  - **Memoized**: the encode costs ~30 ms and sits on the request path of every text search, so `_encode_cached` is an `lru_cache(256)` over the **folded** text (capitalisation variants therefore share one entry) — ~0.1 ms on a hit, bounded at ~3.0 MB. It returns a **tuple** that `generate_embedding` copies into a fresh list per call: the cached value is shared by every caller of a key and callers assign it onto ORM attributes, so handing out the cached object would be a shared mutable with no owner. A hit still pays the `to_thread` hop — microseconds against a 30 ms miss, and one code path rather than two. The cache is shared with document text, so a sweep re-embedding hundreds of media evicts queries; that's the LRU working, not a bug
