...
- `JOBS_SWEEP_ABORT_AFTER_CONSECUTIVE_FAILURES` — default 5; the `update_sweep` circuit breaker (v0.14.13). On this many **consecutive** `upstream_outage` step-1 failures the sweep aborts (raises `TransientUpstreamError` → job failed+retryable, maintenance cleared at once) instead of grinding through every due media at the full retry budget while MAL is down — which held the maintenance window (503 on login) for hours until a restart. A non-upstream failure (stray 404) neither trips nor resets the breaker
