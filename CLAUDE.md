...
- **services/** — Business logic as module-level async functions:
  ...
  - `scrape_dispatcher.py` — handlers for `user_scrape` and `update_sweep` jobs. `user_scrape` wraps the existing `handle_search_mal_api_results` + `save_search_results` pipeline behind a `ProgressReporter` so the bell sees mid-flight progress without waiting for the whole job tx. `update_sweep` tier-selects due anime via `AnimeDAO.select_due_for_sweep`, then for each anime refreshes child media via `JikanScraper.refresh_anime` (`/anime/{id}/full`) and diffs the volatile fields (`score`, `scored_by`, `episodes`, `airing_status`, `aired_to`) via `_apply_media_diff`. **Per-anime commit + per-anime try/except** — a worker crash mid-sweep preserves the already-refreshed anime, and a single bad MAL response fails *that* anime only without aborting the loop. Score and scored_by are bundled through `_weighted_score(score, scored_by) = score * log10(scored_by + 1)` (the same weighted formula used for search ranking) and only count as stability-resetting when `abs(new − old) >= _SCORE_STABILITY_THRESHOLD = 0.05` — without this, a single new vote per night on a million-vote anime would reset the counter forever and the row would never stabilize. None↔value transitions bypass the threshold (first votes coming in is structural). New values are written through to the DB regardless; the threshold gates only the stability *signal*. Always bumps `last_checked_at` on both freshness sidecars; resets `stable_check_count` to 0 when any media is currently airing or any field changed (above threshold), else `min(prev + 1, 99)`.
