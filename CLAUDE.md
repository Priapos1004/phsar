...
- **`AnimeDAO`**:
  ...
  - `score_top_percent(anime_id)` (v0.14.11) — where this anime ranks among all scored anime by its confidence-weighted MAL score (`avg_score * log10(avg_scored_by + 1)`, matching the search-ranking weight) as a "top N%". Single window pass: a per-anime metric CTE → `rank() OVER (ORDER BY metric DESC)` minus 1 (strictly-better count) + `count() OVER ()` (total), so the metric set is scanned once instead of via a twice-referenced scalar subquery; `None` when the anime has no scored media. `MediaDAO.score_top_percent(media_id)` is the per-media analogue (`score * log10(scored_by + 1)`), a single sort-free seq scan over the scored rows (`count(*) FILTER (metric > this) + count(*)`) — a `rank()`/index form was tried and reverted: ranking against all rows reads every scored row regardless, so EXPLAIN showed the planner sorts/scans either way and an expression index just added sweep write-overhead. Both share the SQL formula via `weighted_score_expr` (`search_filters.py`; explicit `log(10, x)`, drift-tested against the Python twin `scrape_dispatcher._weighted_score`) and surface as `score_top_percent` on the detail responses (anime ranks among anime, media among media) → the frontend "Top N%" chip. Both are sub-ms at current scale; cache at sweep time if it ever matters at 10k+ media
