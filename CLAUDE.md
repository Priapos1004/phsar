...
- **`/watchlist`** — per-user watchlist + lists (`require_user_or_admin`, so guests 403). "List" is the UI term for a tag; one tag per entry, one entry per (user, media)
  ...
  - Entries: `PUT`/`GET`/`DELETE /watchlist/media/{media_uuid}` (keyed on media_uuid so the bookmark toggle can call it with what it has), `GET /watchlist/anime/{uuid}`, `PUT /watchlist/bulk` (note applies to ALL selected media), `POST /watchlist/bulk-delete`, `GET /watchlist/items` (wide one-fetch projection for the overview page), `GET /watchlist/media-tags` (bookmark icon-state set + per-media tag color)
