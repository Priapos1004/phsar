...
- **`AnimeDAO`**:
  ...
  - `select_due_media_for_sweep` — **media-level** four-tier sweep selection (v0.14.8): each atom is a direct predicate on the media row + its `MediaFreshness` sidecar (airing now / still stabilizing `stable_check_count < SWEEP_STABILIZE_THRESHOLD` / weekly recent main / long tail), so a still-airing umbrella's stable members are no longer dragged through a refresh every night. v0.15.3 gave the long tail a **per-row window** rather than a fifth tier: `due_long_tail` compares `last_checked` against `SWEEP_ARCHIVAL_DAYS` (180) for media that premiered over `SWEEP_ARCHIVAL_AGE_YEARS` ago and `SWEEP_LONG_TAIL_DAYS` (90) otherwise, via a `CASE` on the interval. One predicate, so the tiers stay independent — a separate fifth branch would need tier 4 narrowed by `not_(archival)` to avoid shadowing it, and that coupling is easy to forget. It also makes NULL correct for free: an undated media fails the `WHEN` and lands on the 90-day `ELSE`. Returns `Media` rows with the parent `Anime` + its full media set eager-loaded (the dispatcher groups by `anime.id`). `LIMIT` bounds MAL calls = media (the real 1 req/s cost unit). Replaced the anime-grained `select_due_for_sweep`. Backed by:
    ...
    - Both are declared at module scope in `models/media.py`, not only in migration `8d4f2a1c9e7b`. That's load-bearing: autogenerate proposes DROPping any index absent from the metadata, and until v0.15.4 these two were missing — one unread `alembic revision` diff away from taking the sweep's access paths with them. `alembic check` in CI is the standing guard
    - NOT the `media_freshness`/`anime_freshness` `last_checked_at` pair — **dropped in v0.15.4** (migration `80bdabf2d417`) and not to be re-added. The planner can't use either (the staleness predicate is a `coalesce` across a joined table, so it isn't sargable, and the `ORDER BY` sits on the nullable side of a LEFT JOIN), and both were worse than inert: `last_checked_at` is the only indexed *mutable* column on either sidecar, so indexing it made every sweep write a non-HOT update (prod: `n_tup_hot_upd = 0` across ~5.1k lifetime updates). The prod-snapshot plan check v0.15.3 gated this on is done — the plan is byte-identical without them (cost `912.03..918.71`, 621 buffers, Seq Scan + quicksort); see `compound-docs/2026-08-06-performance-baseline.md`
...
- **Backend Tests** (`backend-test.yml`): `pytest` against pgvector service container — every push/PR. Also runs **`alembic check`** (v0.15.4) against a throwaway `migrationcheck` DB brought up by `alembic upgrade head`, before the test schema is built. Two guards in one cheap step: models and migrations agree (an index or column declared in only one is what makes the next `--autogenerate` propose a destructive diff), and the chain still replays from empty — the test DB itself is built by `create_all`, so checking *that* would compare metadata against itself and pass no matter how far things drifted
