...
- **main.py** — app factory + lifespan
  - Startup sequence: seeders → embedding backfill → `JobDAO.reap_orphans` → `job_worker.register_dispatcher` (for each of `user_scrape`, `update_sweep`, `seasonal_sweep`, `upcoming_sweep` (same fn as seasonal), `backup`) → `job_worker.start()` → yield → post-yield backfills (**backup self-heal** → optional one-shot full re-embed when `EMBEDDING_REEMBED_ON_STARTUP` → relation-backfiller → merge-candidate backfill → split-candidate backfill). Post-yield backfills run in `asyncio.create_task` so /health responds during the ~14-min relation cold start; split detection runs LAST so it sees relation-backfiller's TERMINAL sidecars and merge-backfiller's collapsed pairs
    - `_ensure_up_to_date_backup` (v0.15.3) enqueues a `backup` job when no dump on disk is both `integrity == ok` **and** `schema_current`. `docker/entrypoint.sh` runs `alembic upgrade head` before the app process, so after a migrating deploy every existing dump carries the previous revision and restoring one would roll the schema back — this closes the window where an install has no restorable backup until the next nightly cron. Post-yield because `list_backups()` may shell out `pg_restore --list` per sidecar-less dump; **first** among the post-yield passes because all of them are idempotent derived-data repairs that re-run every boot, so a pre-backfill dump loses nothing while a post-backfill one couldn't recover from a bad backfill. `schema_current is None` (legacy sidecar) does NOT satisfy the predicate; a `JobDAO.count_active_by_kind` guard stops rapid restarts stacking dumps; runs as a `cron`-source system job so it needs no new `BackupSource` member and stays out of every user's bell
